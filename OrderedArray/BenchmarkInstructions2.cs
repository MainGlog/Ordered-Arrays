...
{
	public class BenchmarkInstructions2
	{
		public enum Op
		{
			Insert,
		...
		private BenchmarkInstructions2() { }
		private BenchmarkInstructions2(Op instruction) => (Instruction) = (instruction);
		private BenchmarkInstructions2(Op instruction, int number) => (Instruction, Number) = (instruction, number);
		private BenchmarkInstructions2(Op instruction, int number, int index) =>
			(Instruction, Number, Index) = (instruction, number, index);

		public static List<BenchmarkInstructions2> GenerateInstructions(int amount = 120000, int seed = 10)
		{
			return GenerateInstructions(null);
		}

		public static List<BenchmarkInstructions2> GenerateInstructions(Op[]? opsToIgnore, int amount = 120000, int seed = 10)
		{
			Random rand = new Random(seed);
			List<BenchmarkInstructions2> instructions = new List<BenchmarkInstructions2>(amount);
			...
			for (int i = 0; i < amount; i++)
			{
				if (i < amount * 0.11f)//Insert
				{
					...
					instructions.Add(new BenchmarkInstructions2(Op.Insert, randomNumber));
				...
				else
				{
					...
					else if (opChoice < 30)//Insert Into
					{
						...
						instructions.Add(new BenchmarkInstructions2(Op.InsertInto, randomNumber, randomIndex));
						numbersInserted.Add(randomNumber);
						stats[1]++;
					}
					else if (opChoice < 50)//Search for item definitely in the list
					{
						...
						instructions.Add(new BenchmarkInstructions2(Op.Search, randomNumber));
						stats[2]++;
					}
					...
					else if (opChoice < 79)//Remove
					{
						...
						instructions.Add(new BenchmarkInstructions2(Op.Remove, randomNumber));
						stats[3]++;
					}
					else if (opChoice < 99)//Remove at
					{
						...
						instructions.Add(new BenchmarkInstructions2(Op.RemoveAt, 0, randomIndex));
						stats[4]++;
					}
					else//Clear
					{
						...
						instructions.Add(new BenchmarkInstructions2(Op.Clear));
		...
		public static List<BenchmarkInstructions2> GenerateInstructions(Op instruction, IList<int>? list = null, int amount = 50000, int seed = 10)
		{
			Random rand = new Random(seed);
			List<BenchmarkInstructions2> instructions = new List<BenchmarkInstructions2>(amount);
			List<int> numbersInserted = [];
			...
			switch (instruction)
			{
				...
				case Op.InsertInto:
					randomNumber = rand.Next(int.MinValue, int.MaxValue);
					instructions.Add(new BenchmarkInstructions2(Op.InsertInto, randomNumber, 0));
